...
class Log:
    ...
    async def _set_channel(self):
        ...
        if config is None or config["channel"] is None:
            return
        self.channel: discord.TextChannel = await self.guild.get_channel(int(config["channel"]))

    async def log(self, type: str, user: discord.User, mod: discord.User, *, reason: str):
        if self.channel is None:
            return f"No Log Channel has been setup for {self.guild.name}"
        else:
            embed = discord.Embed()
            embed.set_author(name=f"{type} | {user.name}#{user.discriminator}")
            embed.add_field(name="User", value=f"<@{user.id}> `({user.name}#{user.discriminator})`")